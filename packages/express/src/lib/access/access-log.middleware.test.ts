...
it('logs the elapsed time', async () => {
  ...
  expect(logger.access).toHaveBeenCalledWith(expect.objectContaining({ time: expect.any(Number) }));
});
...
it('logs the client ip', () => {
  ...
  expect(logger.access).toHaveBeenCalledWith(expect.objectContaining({ clientIp: '10.10.10.10' }));
});
...
it('logs the user agent', async () => {
  ...
  expect(logger.access).toHaveBeenCalledWith(expect.objectContaining({ userAgent: 'test-agent' }));
});
