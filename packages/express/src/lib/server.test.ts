...
describe('init', () => {
  ...
  it('applies the pre mount hook before mount', async () => {
    const server = new CustomServer({
      pre(app) {
        app.use((req, res) => res.json({ pre: 'ok' }));
      },
      mountApp(app) {
        app.get('/mount', (req, res) => res.send({ mount: 'ok' }));
      },
    });
    server.init();
  ...
  it('applies the post mount hook after mount', async () => {
    const server = new CustomServer({
      post(app) {
        app.use((req, res) => res.json({ post: 'ok' }));
      },
      mountApp(app) {
        app.get('/mount', (req, res) => res.send({ mount: 'ok' }));
      },
...
describe('start', () => {
  it('listens on default ports', () => {
    ...
    vi.spyOn(server.metricsHttpServer, 'listen').mockImplementation(() => server.metricsHttpServer);
